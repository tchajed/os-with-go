...
## Background: Load Balancing and Preemption in Parallel Systems

A parallel system with P processors and a collection of tasks faces a fundamental scheduling question: which processor should run which task? The simplest answer -- static partitioning, where tasks are divided evenly among processors at the start -- works only when you know the tasks' durations in advance and they are roughly equal. In practice, tasks vary wildly in cost. A web server might handle one request in 50 microseconds and the next in 500 milliseconds. A parallel divide-and-conquer algorithm generates subtasks whose sizes depend on the input data. Static partitioning leaves some processors idle while others are overwhelmed, and the overall completion time is dictated by the slowest processor. The problem gets worse as processor counts grow: on a 64-core machine, even modest imbalance means most of the hardware sits idle.

Work stealing is the dominant solution to this load-balancing problem, and its theoretical foundations were laid by Robert Blumofe and Charles Leiserson in their 1999 paper "Scheduling Multithreaded Computations by Work Stealing." The key idea is elegant: each processor maintains its own local deque (double-ended queue) of tasks. When a processor finishes its current work, it first checks its own deque. If it is empty, the processor becomes a *thief* and picks a random victim processor to steal from, taking tasks from the opposite end of the victim's deque. Blumofe and Leiserson proved that for a computation with total work T1 and critical path length T-infinity, a work-stealing scheduler completes execution in expected time T1/P + O(T-infinity), which is provably close to optimal. Their Cilk system at MIT demonstrated these ideas in practice and showed that stealing from the "old" end of the deque (the larger, higher-level tasks) minimizes both the number of steals and the communication overhead. The "steal half" variant, where the thief takes half the victim's queue, further reduces the number of steal operations needed to balance load, converging in O(log n) steals.

Work stealing has become ubiquitous in systems software. Java's `ForkJoinPool`, introduced in Java 7 (2011), brought work stealing to mainstream concurrent programming, implementing the fork/join paradigm where tasks recursively split into subtasks that are distributed across worker threads with per-thread deques. Intel's Threading Building Blocks (TBB) and Cilk Plus used the same approach for C++ parallelism. In the Rust ecosystem, the Tokio async runtime uses a work-stealing scheduler where each worker thread has a local queue of async tasks, and idle workers steal from busy ones -- a design that provides good latency and throughput for I/O-heavy server workloads. Even the Linux kernel's Completely Fair Scheduler (CFS) uses a form of work stealing for its multi-core load balancer: each CPU has its own run queue, and the kernel periodically rebalances by pulling tasks from the busiest CPUs to the least busy ones, organized through a hierarchy of scheduling domains that reflect the machine's NUMA topology. The pattern is the same everywhere -- distribute state for fast-path performance, steal for balance.

Work stealing solves the problem of where to run tasks, but a complementary problem remains: how long should a task run before the scheduler reclaims the processor? This is the problem of *preemption*. Early operating systems like classic Mac OS and Windows 3.1 used cooperative scheduling, where programs voluntarily yielded the processor. This was simple but fragile -- a single misbehaving or compute-intensive program could freeze the entire system. Modern operating systems use preemptive scheduling, typically driven by hardware timer interrupts. Linux, for example, offers multiple preemption levels through its `CONFIG_PREEMPT` settings: from no preemption (only at explicit schedule points, suitable for throughput-oriented servers) to full kernel preemption (suitable for latency-sensitive desktop and real-time workloads). The tension between cooperative and preemptive scheduling is not just historical -- it reappears in every user-space runtime that multiplexes its own tasks onto OS threads, including Go, Erlang, and async runtimes in Rust and JavaScript.

This module examines how Go's runtime addresses both problems. Go's scheduler implements work stealing across per-P local run queues, with a lock-free steal-half algorithm directly inspired by the Blumofe-Leiserson framework. For preemption, Go takes a layered approach: cooperative preemption via stack-growth checks at function prologues (present since Go 1.2), and asynchronous signal-based preemption via `SIGURG` (added in Go 1.14 to handle tight loops without function calls). Overseeing everything is the `sysmon` thread -- a watchdog goroutine that runs without a P and enforces 10ms time slices, reclaims processors from goroutines stuck in system calls, and polls the network. Together, these mechanisms form a scheduler that is both highly concurrent and fair, even in the face of uncooperative workloads.
