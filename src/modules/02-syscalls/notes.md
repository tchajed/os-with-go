...
## Background: The User-Kernel Boundary

System calls are the fundamental interface between user programs and the operating system kernel. Every time a program opens a file, allocates memory, sends a network packet, or creates a new process, it must cross the boundary from user mode into kernel mode through a system call. This boundary is not merely a software convention -- it is enforced by the CPU's hardware privilege levels. The kernel runs in a privileged mode (ring 0 on x86, EL1 on ARM64) with full access to hardware and memory, while user programs run in a restricted mode (ring 3 / EL0) where attempts to execute privileged instructions or access protected memory trigger a processor fault. System calls are the controlled gateway through this hardware barrier.

The mechanism for crossing this boundary has evolved considerably over the decades. Early systems like the PDP-11 used a `TRAP` instruction to transfer control to the kernel, and this basic model -- a special instruction that atomically switches privilege levels and jumps to a kernel-defined entry point -- has persisted in every architecture since. On x86, the history is particularly convoluted: the original 8086 used `INT 0x80` (a software interrupt), which required the CPU to look up an interrupt descriptor table entry, save state to the stack, and perform a privilege level change -- a process that could take hundreds of CPU cycles on modern processors due to pipeline flushes and serialization. Intel introduced `SYSENTER`/`SYSEXIT` with the Pentium II, and AMD introduced `SYSCALL`/`SYSRET` with the K6-2, both designed to reduce this overhead by using pre-configured Model-Specific Registers (MSRs) instead of memory-based descriptor tables. On modern x86-64 Linux, `SYSCALL` is the standard mechanism, bringing the transition cost down to roughly 50-100 nanoseconds -- fast, but far from free when called millions of times per second.

Linux today defines over 450 system calls on x86-64 (the exact count grows with each kernel release as new calls are added and old ones are rarely removed, since the syscall ABI is a stability guarantee). These calls fall into several broad categories: process management (`fork`, `clone`, `execve`, `wait4`, `exit`), file I/O (`open`, `read`, `write`, `close`, `lseek`), memory management (`mmap`, `munmap`, `brk`, `madvise`), networking (`socket`, `bind`, `listen`, `accept`, `sendto`, `recvfrom`), and inter-process communication (`pipe`, `shmget`, `semop`). Over the years, many syscalls have been superseded by improved versions -- `open` by `openat`, `epoll_wait` by `epoll_pwait2`, `clone` by `clone3` -- reflecting the kernel community's preference for extending the interface with new calls rather than modifying the semantics of existing ones. Newer additions like `io_uring_setup` (added in Linux 5.1) and `landlock_add_rule` (Linux 5.13) show that the syscall table continues to grow as the kernel gains capabilities.

The cost of crossing the user-kernel boundary has driven significant innovation in reducing or avoiding syscalls altogether. The vDSO (virtual Dynamic Shared Object) is a kernel-provided shared library mapped into every process's address space, allowing frequently-called operations like `clock_gettime` and `gettimeofday` to execute entirely in user space by reading kernel-maintained data pages -- turning a ~100ns syscall into a ~20ns function call. At the other extreme, frameworks like DPDK (Data Plane Development Kit) bypass the kernel entirely for network I/O, mapping NIC hardware directly into user space to achieve line-rate packet processing. Linux's `io_uring`, introduced in kernel 5.1, takes a middle path: it uses shared memory ring buffers between user space and the kernel to batch and submit I/O operations asynchronously, dramatically reducing the per-operation syscall overhead for I/O-heavy workloads. These approaches highlight a recurring tension in OS design: syscalls provide safety and abstraction, but that safety comes at a performance cost that matters in high-throughput systems.

This module examines how Go navigates this landscape. Go makes an unusual choice among programming languages: on Linux, the runtime bypasses libc entirely and issues system calls directly via assembly stubs, relying on the kernel's stable syscall ABI. Most other languages -- and even Go itself on macOS and Windows -- call through the C library, which serves as a portability layer and insulates programs from kernel interface changes. Go's approach on Linux yields tighter integration between the runtime scheduler and syscall handling (as we will see with `entersyscall`/`exitsyscall`), but it also means the runtime must handle the raw kernel interface, including error conventions, signal safety, and architectural differences. We will trace this path from the raw `SYSCALL` instruction in assembly, through the `syscall` package's Go wrappers, to the runtime's scheduler integration that keeps goroutines running efficiently even when some are blocked in the kernel.
