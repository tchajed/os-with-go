...
## Background: The Evolution of Synchronization

The fundamental challenge of concurrent programming is deceptively simple to state: when multiple threads of execution share mutable state, their interleaved operations can produce results that no sequential execution would. A counter incremented by two threads can lose updates; a linked list modified concurrently can develop cycles or dangling pointers; a partially-written data structure can be observed in a state that its author never intended. This problem is as old as multiprogramming itself, and the history of systems software is in many ways the history of increasingly sophisticated answers to it.

The formal study of synchronization begins with Edsger Dijkstra's 1965 paper introducing the semaphore, a counting primitive with two operations -- P (wait) and V (signal) -- sufficient to solve mutual exclusion and coordination problems. Dijkstra's work established the intellectual framework, but practical implementations required hardware support. Early processors offered test-and-set instructions; later architectures introduced compare-and-swap (CAS), first appearing on the IBM System/370 in 1970, which became the workhorse of lock-free programming. Modern hardware provides further refinements: x86 processors implement Total Store Order (TSO), a relatively strong memory model where stores are seen in program order by other cores, while ARM and RISC-V use weaker ordering models that demand explicit barriers or load-acquire/store-release instructions. These architectural differences mean that correct synchronization code is not portable at the instruction level -- a lesson that has motivated language-level memory models like C11's and Go's.

A pivotal innovation in the Linux ecosystem was the futex (fast userspace mutex), introduced by Hubertus Franke, Matthew Kirkwood, and Rusty Russell in 2002 and merged into Linux 2.5.7. The key insight was that most lock acquisitions are uncontended: if a thread can acquire a lock with a single atomic operation in user space, there is no reason to enter the kernel at all. The futex syscall is invoked only on the slow path -- when a thread must sleep because the lock is held, or when it must wake a sleeping waiter. This design eliminated the overhead of a syscall on every lock/unlock cycle, and it remains the foundation of pthread mutexes, condition variables, and many user-space synchronization libraries on Linux to this day. The design also inspired similar approaches on other platforms, though macOS takes a different path using Mach semaphores and pthread primitives.

Real-world synchronization involves tradeoffs that go beyond correctness. Fairness and throughput are often in tension: a lock that always hands ownership to the longest-waiting thread (strict FIFO) prevents starvation but suffers from "lock convoy" effects and poor cache locality, while a lock that lets any thread grab ownership (barging) maximizes throughput at the risk of starving unlucky waiters. The evolution from simple spinlocks to sleeping locks to adaptive mutexes -- which spin briefly before sleeping, as seen in Linux's mutex implementation and Windows's SRWLOCK -- reflects decades of effort to balance these concerns. Priority inversion, famously diagnosed in the 1997 Mars Pathfinder mission when a low-priority thread holding a bus mutex blocked the high-priority watchdog task, led to the widespread adoption of priority inheritance protocols. Reader-writer locks introduce their own design space: Linux's percpu_rwsem optimizes the common read path by using per-CPU counters, accepting a heavyweight write-side operation in exchange. Lock-free and wait-free data structures, built directly on hardware atomics, eliminate blocking entirely but are notoriously difficult to design correctly and do not always outperform well-tuned locks under real workloads.

Go's synchronization stack reflects all of these lessons. It is built in layers: hardware atomics at the bottom, OS primitives (futexes on Linux, semaphores on macOS) above them, a runtime mutex for the scheduler's own internal use, and finally the `sema.go` semaphore that powers `sync.Mutex`, `sync.RWMutex`, and other user-visible primitives. Go's `sync.Mutex` even implements an explicit starvation mode inspired by the fairness-vs-throughput tradeoff -- it barges under low contention for throughput but switches to direct handoff after a goroutine has waited more than 1ms. This module walks through each layer from the bottom up, showing how these abstractions compose and where the hard-won lessons of synchronization history appear in production code.
